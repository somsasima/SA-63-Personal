...
// EvalQuery return f(ctx, q).
func (f JobtitleQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.JobtitleQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.JobtitleQuery", q)
}

// The JobtitleMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type JobtitleMutationRuleFunc func(context.Context, *ent.JobtitleMutation) error

// EvalMutation calls f(ctx, m).
func (f JobtitleMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.JobtitleMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.JobtitleMutation", m)
}
